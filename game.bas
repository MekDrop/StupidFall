...
End Type
Screen 19, 8
Dim Platforms(1 to 21) As Platform, Fx As Integer, K As String, Speed As Integer, Score As Integer, Loose As Integer, I As Integer, X As Integer, Y As Integer
Speed = 1
Do    
    K = Inkey 
    If Loose = 0 Then
        ...
        For I = LBound(Platforms) To UBound(Platforms)
            If Platforms(I).Y1 > (600 + 2 + Platforms(I).W) Then
                ...
                Platforms(I).Y1 = 0
                Score = Score + I
                Speed = IIf( ( Fix(Score / (231 * Speed)) + 1) > Speed, Speed + 1, Speed)
        ...
        Fx = Fx + IIf(Len(K) > 1, IIf((Right(K, 1) = "M") And (Fx < 780), 20, IIf((Right(K, 1) = "K") And (Fx >= 20), -20, 0)), 0)    
        Line (800, 600)-(0, 580), 2, BF
        WindowTitle "Stupid Fall 0.1.2 : Score - " & Str(Score) & " : Speed - " & Str(Speed) & " : ESC - Exit "
    Else
